  ...
  modal.innerHTML = `
    ...
    <div class="chat-modal-container">
      <div class="chat-modal-header">
        <div>
          <div class="chat-modal-title">${escapeHtml(otherName)}</div>
          <div class="chat-modal-subtitle">${escapeHtml(productTitle)}</div>
        </div>
        <button class="chat-modal-close" onclick="closeChatModal()"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg></button>
      </div>
  ...
  if (messages.length === 0) {
    container.innerHTML = `
      <div class="chat-empty">
        <div style="margin-bottom: 0.75rem; color: var(--text-dim);"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/></svg></div>
...
// escapeHtml() está definido en app.js (compartido en todas las páginas)
